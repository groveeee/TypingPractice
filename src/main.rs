...
*/
fn getData() -> String {
    let resp = reqwest::blocking::get("http://www.atoolbox.net/Api/GetLoremIpsum.php?p=5").unwrap().text().unwrap();
    resp.replace("<p>", "").replace("</p>", "").replace("\n", "").clone()
}
...
渲染UI界面
 **/
fn ui<B: Backend>(f: &mut Frame<B>, string: &mut String, textarea: &mut TextArea, user_input: &mut String,cool:&mut CoolView,b:bool) {
    // 布局
    let chunks = Layout::default() // 首先获取默认构造
        ...
        ].as_ref()) // 按照 10%划分  划分了8个区域 再加一个20%的文本编辑框
    ...
    if b {
        user_paragraph = Paragraph::new(Span::styled(
            content,
            Style::default().fg(Color::LightGreen).add_modifier(Modifier::SLOW_BLINK),// 设置字体样式
        ))
            .block(Block::default().borders(Borders::ALL).border_style(Style::fg(Default::default(), Color::LightGreen)).title("TypingPractice OK"))//设置区块标题
        ...
        user_paragraph = Paragraph::new(Span::styled(
            content,
            Style::default().fg(Color::LightRed).add_modifier(Modifier::SLOW_BLINK),// 设置字体样式
        ))
            .block(Block::default().borders(Borders::ALL).border_style(Style::fg(Default::default(), Color::LightRed)).title("TypingPractice ERR"))//设置区块标题
            .alignment(tui::layout::Alignment::Center);
    }
    ...
    let chunks = Layout::default()
        ...
        .constraints([Constraint::Percentage(38), Constraint::Percentage(28),Constraint::Percentage(38)].as_ref())
        .split(chunks[4]);

    // 正确的
    let left_paragraph = Paragraph::new(Span::styled(
        "PRECISE:".to_string()+ &*cool.precise_count.to_string(),
        Style::default().fg(Color::Green).add_modifier(Modifier::BOLD),// 设置字体样式
    ))
        .block(Block::default().borders(Borders::ALL).border_style(Style::fg(Default::default(), Color::Green)).title("PRECISE NUM"))//设置区块标题
    ...
    let center_paragraph = Paragraph::new(Span::styled(
        ...
        .block(Block::default().borders(Borders::ALL).border_style(Style::fg(Default::default(), Color::LightBlue)).title("SPEED"))//设置区块标题
        .alignment(tui::layout::Alignment::Center);// 对齐方式
    ...
    let right_paragraph = Paragraph::new(Span::styled(
        ...
        .block(Block::default().borders(Borders::ALL).border_style(Style::fg(Default::default(), Color::Red)).title("MISTAKE NUM"))//设置区块标题
        .alignment(tui::layout::Alignment::Center);// 对齐方式
